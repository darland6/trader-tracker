...
The following skills are available for portfolio management tasks. Use `/skill-name` to invoke:

### /reconcile - Portfolio Reconciliation
Reconcile portfolio data with actual brokerage positions. Handles:
- **Schwab CSV Import**: Import transaction history from Schwab exports
- **Position Cleanup**: Remove fractional shares, close stale positions
- **Option Reconciliation**: Fix option position mismatches, expire old options
- **Cash Adjustment**: Create ADJUSTMENT events to match exact account balance

**Usage**: Ask to reconcile portfolio with a screenshot or CSV export

### /llm-debug - LLM Configuration Diagnostics
Diagnose LLM connection issues (local LM Studio, Ollama, or Claude API):
- **Config Check**: Verify .env and llm_config.json settings
- **Connectivity Test**: Test server reachability and model availability
- **Chat Test**: Verify chat completion endpoint works

**Quick diagnostic**: `python scripts/llm_debug.py`

**Key insight**: `.env` file settings OVERRIDE `llm_config.json` - always check both!
...
| `scripts/setup_portfolio.py` | Portfolio import/setup wizard |

---

*Generated by Claude Code - Last updated: 2026-01-10*
